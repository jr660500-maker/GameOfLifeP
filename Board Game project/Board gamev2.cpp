...
void player::changehome(int newHomeValue){
    ...
    *homeValue=homeValueL[newHomeValue];
    delete[] homeNameL;
    delete[] homeValueL;
...
void board::history(){
    ...
    for (int i = 0; i < players; i++) {
        playerSet.insert(p[i]);
    }
    for(player x:playerSet){
        historyFile << "Player Name: " << x.viewname() << endl;
        historyFile << "Job: " << x.viewjob() << endl;
        historyFile << "Money: $" << x.viewmoney() << endl;
        historyFile << "Position: " << x.viewposition() << endl;
        historyFile << "Lifepoints: " << x.viewlifepoints() << endl;
        historyFile << "Total Score: " << x.totalScore() << endl;
        historyFile << "Home: " << x.viewhome() << endl;
        historyFile << "-----------------------------" << endl;
    }
